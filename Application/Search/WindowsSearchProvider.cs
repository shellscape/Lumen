...
namespace Lumen.Search {
  ...
  public static class WindowsSearchProvider {
	...
	public static List<WindowsSearchResult> Search(string term) {
	  ...
	  // While this works for the GROUP ON clause, it's horrendous and the performance is worse than pulling 2000 records.

//	  var sql = @"
//		group on System.ItemNameDisplay over(
//		  select top 1000 
//		  System.ItemNameDisplay, System.ItemPathDisplay, System.Kind, System.Search.Rank, System.FileAttributes 
//		  from systemindex 
//		  where CONTAINS(""System.ItemNameDisplay"", '""*{0}*""')
//		  and System.FileAttributes <> ALL BITWISE 0x4 and System.FileAttributes <> ALL BITWISE 0x2 
//		)";	  
	  
	  //DataSet dataset = new DataSet();

	  //using(OleDbDataAdapter adapter = new OleDbDataAdapter(string.Format(sql, term.Trim()), _connection)) {
	  ...
	  //  var resultRow = dataset.Tables[0].Rows[i];
	  //  String fileName = resultRow["System.ItemNameDisplay"].ToString();
	  //  int chapter = resultRow.Field<int>("Chapter");
	  //  var data = (from row in dataset.Tables[1].AsEnumerable() 
	  //			  where row.Field<int>("TableChapter") == chapter 
	  //			  orderby row.Field<int>("System.Search.Rank") descending 
	  //			  select row).First();

	  //  //Debug.WriteLine(row[0].ToString() + " - " + row[1].ToString() + " - " + data[0].ToString());

	  //  var result = new WindowsSearchResult() {
	  //	FileName = data[0].ToString(),
	  //	FilePath = data[1].ToString()
	  //  //	Rank = (int)subreader["System.Search.Rank"]
	  //  };
	  ...
	  //			  group item by new { item.FileName } into @group
